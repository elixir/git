...
char *mingw_getenv(const char *name)
{
	...
	/* use original getenv if not initialized yet */
	if (!environ_size)
		return getenv(name);
...
void mingw_startup()
{
	 ...
	 */
	tmpenv = NULL;
	ALLOC_GROW(tmpenv, (i + 1) * sizeof(char*), environ_alloc);
	...
	qsort(tmpenv, i, sizeof(char*), compareenv);

	/* finally assign new environment */
	tmpenv[i] = NULL;
	environ_size = i + 1;
	environ = tmpenv;
